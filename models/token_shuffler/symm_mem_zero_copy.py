...
# ────────────────────────────────────────────────
# EP Token Shuffler via Symmetric Memory (Optimized: Mem Pool & Zero Copy)
# ────────────────────────────────────────────────
class ZeroCopySymmMemTokenShuffler(MoETokenShuffler):
    """
    ...
    Memory pool and zero-copy optimized.
    """
    def __init__(self, module_of_experts, E, K, ep_group):
        ...
        self._symm_mem_pool = symm_mem.get_mem_pool(torch.cuda.current_device())
        self.out_minlen = int(self.EPR*16)

    def use_symm_mem_pool_ctx_fn(self):
    ...
    def permute_for_dispatch(self, tokens: torch.Tensor, k_eids: torch.Tensor, k_weights: torch.Tensor):
        ...
        with self.use_symm_mem_pool_ctx_fn():
    ...
    def dispatch(self, tokens, expert_splits):
        ...
        expert_major_tokens, self.out_so = ZeroCopySymmMemDispatchFunc.apply(
            tokens, expert_splits, self.ep_group.group_name,
            self.max_in, self.max_out, self._symm_mem_pool
        )
    ...
    def combine(self, expert_computed_tokens):
        return ZeroCopySymmMemCombineFunc.apply(
                expert_computed_tokens, self.out_so, self.ep_group.group_name, 
                self.max_out, self.max_in, self._symm_mem_pool
            )
    
    def apply_router_probs(self, tokens_symmem, probs):
        return MulProbsFunc.apply(tokens_symmem, probs, self._symm_mem_pool)
...
class ZeroCopySymmMemDispatchFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_out, _grad_so):
        ...
        torch.ops.symm_mem.all_to_all_vdev_2d_offset(
            grad_out, grad_inp_symm, in_so, out_so_bwd, ctx.group_name
        )
...
class ZeroCopySymmMemCombineFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_out, _grad_out_so):
        ...
        torch.ops.symm_mem.all_to_all_vdev_2d(
            grad_out, grad_inp_symm, in_splits, bwd_so, ctx.group_name,
            major_align=1,
        )
...
class MulProbsFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, tokens_symmem, probs, symm_mem_pool):
    ...
    def backward(ctx, grad_out):
        ...
        return grad_tokens_symmem, grad_probs, None